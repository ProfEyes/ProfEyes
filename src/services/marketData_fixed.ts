...
async function getBinanceData(symbol: string): Promise<{ ticker: any; klines: any[] } | null> {
  try {
    ...
    try {
      ...
      // Se n�o temos dados em cache ou est�o desatualizados, buscar da API
      try {
        // Obter ticker atual
        ...
        // Intervalo de 1h para os �ltimos 500 candles
        ...
        try {
          // Usar klinesData que j� est� declarada no escopo externo
          const cryptoDataToInsert = klinesData.map((kline: any[]) => {
            try {
              ...
              return {
                ...
                price_change: parseFloat(kline[4]) - parseFloat(kline[1]),
                price_change_percent: ((parseFloat(kline[4]) - parseFloat(kline[1])) / parseFloat(kline[1])) * 100
              };
            } catch (parseError) {
...
function calculateSuccessRate(data: Array<{ close: string; volume: string; high: string; low: string }>): number {
  try {
    ...
    if (rsi > 30 && rsi < 40) score += 15; // Poss�vel revers�o de baixa
    if (rsi > 60 && rsi < 70) score += 15; // Poss�vel revers�o de alta
